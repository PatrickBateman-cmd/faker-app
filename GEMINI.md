...
Comprehensive code audit identified 72 issues. Organized into 4 phases.

### Phase 1 — Critical: Security & Stability

| # | Issue | Where | Fix |
|---|---|---|---|
| 1.1 | **SQL injection** — 13+ f-string SQL interpolations | `dataset_service.py`, `transform_service.py`, `export_service.py`, `generation_engine.py` | Validate column/table names against `^[a-zA-Z_][a-zA-Z0-9_]*$` or whitelist via `information_schema.columns` with `?` params |
| 1.2 | **DuckDB singleton not thread-safe** | `database.py:10-86` | Add `threading.Lock` around `execute()` |
| 1.3 | **Row-by-row INSERT** — 100K `db.execute()` calls/dataset | `generation_engine.py:204-208` | Replace with `db.executemany()` |
| 1.4 | **XXE vulnerability** in XML parsing | `template_library.py:64` | Switch to `defusedxml.ElementTree` |
| 1.5 | **Path traversal via dataset name** | `export_service.py:22-27` | Sanitize filenames, use `os.path.basename()` |
| 1.6 | **Temp files never cleaned up** | `export_service.py:10-54` | `BackgroundTasks` cleanup or streaming response |
| 1.7 | **Open table access** via `/datasets/table/{name}/rows` | `datasets.py:33-39` | Restrict to `metadata_datasets` or remove |
| 1.8 | **Bare `except` swallowing errors** | `financial_service.py:38`, `generation_engine.py:158` | Log + catch specific types |

### Phase 2 — Performance

| # | Issue | Where | Fix |
|---|---|---|---|
| 2.1 | **Row-by-row INSERT** (same as 1.3) | Phase 1 | `executemany()` |
| 2.2 | **New `httpx.Client()` per request** | `iso20022_service.py:88-91` | Module-level singleton with connection pooling |
| 2.3 | **Templates re-read from disk on every call** | `template_library.py:93-107` | In-memory cache with mtime invalidation |
| 2.4 | **Frontend refetches every 5s** | `ResultsViewer.tsx:16` | 30s interval + `refetchIntervalInBackground: false` |
| 2.5 | **`get_all_messages()` loads all into memory** | `iso20022_service.py:191-203` | Max page cap (50) |
| 2.6 | **`lru_cache` cleared on every XSD parse** | `iso20022_service.py:444` | Remove `_cache_clear()` |

### Phase 3 — Architecture & Code Quality

| # | Issue | Where | Fix |
|---|---|---|---|
| 3.1 | **Metadata sync deletes all non-atomically** | `template_library.py:110-120` | Transaction + `INSERT OR REPLACE` |
| 3.2 | **Auth settings declared but not implemented** | `config.py:5-13` | Implement JWT or remove settings |
| 3.3 | **Dead code `_SHARED_KEY_CACHE`** | `generation_engine.py:20` | Remove |
| 3.4 | **`strict=False` in `zip(columns, row)`** | `dataset_service.py:79` | `strict=True` |
| 3.5 | **Inconsistent error handling across routers** | All routers | Global exception handler |
| 3.6 | **Nested/XSD fields silently truncated** | `iso20022_service.py:285,378` | Remove caps or log warning |
| 3.7 | **`generate_from_xsd` hard-codes 10 rows** | `iso20022_service.py:478` | Accept `count` parameter |
| 3.8 | **`import random` inside loop** | `iso20022_service.py:488-544` | Move to module top |

### Phase 4 — Polish & UX

| # | Issue | Where | Fix |
|---|---|---|---|
| 4.1 | **UUID truncated to 8 chars** | `generation_engine.py:136`, `transform_service.py:62` | Full UUID or sequence |
| 4.2 | **`strict=False` in more places** | `dataset_service.py:79,104` | `strict=True` |
| 4.3 | **Export dropdown UX** | `ResultsViewer.tsx:140-163` | Click toggle instead of hover |
| 4.4 | **Duplicated TypeScript types** | `types/template.ts` vs `types/generation.ts` | Consolidate |
...
| 4.7 | **`onResult` uses `unknown`** | `AggregationPanel.tsx:23` | Typed response |
| 4.8 | **Delete mutation lacks error UI** | `ResultsViewer.tsx:27-36` | `onError` handler |
| 4.9 | **Financial panel fetches on mount** | `FinancialPanel.tsx:52-73` | Gate behind Search click |
| 4.10 | **Health check fails if DuckDB down** | `health.py:8-12` | Return degraded status |

**Total effort: ~6–9 hours across ~13 files.** Full details in `REFACTOR.md`.
